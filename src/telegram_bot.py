...
"""
    ...
    def _upload_progress_sync(self, status_msg, current, total, progress_state):
        """Upload progress callback for large files (sync wrapper)"""
        if status_msg and total and progress_state:
            percent = int(current * 100 / total)
            # Update every 20% to avoid too many edits
            if percent >= progress_state['last_percent'] + 20:
                progress_state['last_percent'] = percent
                # Schedule the async update without waiting
                task = asyncio.create_task(self._upload_progress_async(status_msg, percent))
                # Store task reference to prevent garbage collection
                if not hasattr(self, '_progress_tasks'):
    ...
    async def upload_audio(self, chat_id: int, file_path: Path, status_msg=None):
        ...
        try:
            ...
            # Initialize progress tracking state for this upload
            ...
            await self.bot.send_file(
                ...
                progress_callback=lambda c, t: self._upload_progress_sync(status_msg, c, t, progress_state) if status_msg else None
            )
